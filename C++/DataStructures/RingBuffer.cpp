...
public:
    ...
    // reads and removes oldest item, empty returns nullptr
    std::unique_ptr<T> read()
    {
        if (empty()) return nullptr;

        auto result = std::make_unique<T>(std::move(data_[head_]));
        head_ next(head_);
        --size_;
    ...
    // reads into an output parameter, empty returns false
    bool try_read(T& out)
    {
